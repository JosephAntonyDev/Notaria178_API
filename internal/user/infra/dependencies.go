...
func SetupDependencies(r *gin.Engine, db *sql.DB, jwtSecret string, audit ports.AuditLogger) {
	...
	createUserUseCase := app.NewCreateUserUseCase(userRepo, hasher, audit)
	loginUserUseCase := app.NewLoginUserUseCase(userRepo, hasher, tokenManager, audit)
	...
	updateEmployeeUseCase := app.NewUpdateEmployeeUseCase(userRepo, hasher, audit)
